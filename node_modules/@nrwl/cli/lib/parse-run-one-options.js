...
const fs = require("fs");
function calculateDefaultProjectName(cwd, root, wc) {
    ...
    if (relativeCwd) {
        ...
        const matchingProject = Object.keys(wc.projects).find((p) => {
            ...
            return (relativeCwd == projectRoot || relativeCwd.startsWith(`${projectRoot}/`));
        });
        if (matchingProject)
...
function parseRunOneOptions(root, workspaceConfigJson, args) {
    var _a;
    const defaultProjectName = calculateDefaultProjectName(process.cwd(), root, workspaceConfigJson);
    ...
    // we need both to be able to run a target, no tasks runner
    const p = workspaceConfigJson.projects && workspaceConfigJson.projects[project];
    if (!p)
        return false;
    let targets;
    if (typeof p === 'string') {
        targets = JSON.parse(fs.readFileSync(`${p}/project.json`).toString()).targets;
    }
    else {
