...
function createLibraryExecutor(initializeNgPackagr) {
    return function (options, context) {
        ...
        if (options.watch) {
            return rxjs_for_await_1.eachValueFrom(rxjs_1.from(initializeNgPackagr(options, context, dependencies)).pipe(operators_1.switchMap((packagr) => packagr.watch()), operators_1.tap(() => updatePackageJson()), operators_1.mapTo({ success: true })));
        }
        return rxjs_1.from(initializeNgPackagr(options, context, dependencies))
