...
function insertAfterLastOccurrence(nodes, toInsert, file, fallbackPos, syntaxKind) {
    ...
    if (syntaxKind) {
        lastItem = find_nodes_1.findNodes(lastItem, syntaxKind).sort(nodesByPosition).pop();
    }
...
function findClass(source, className, silent = false) {
    const nodes = get_source_nodes_1.getSourceNodes(source);
...
function getImport(source, predicate) {
    const allImports = find_nodes_1.findNodes(source, ts.SyntaxKind.ImportDeclaration);
...
function readJsonInTree(host, path) {
    ...
    try {
        return devkit_1.parseJson(host.read(path).toString('utf-8'));
    }
    catch (e) {
...
function getProjectGraphFromHost(host) {
    return project_graph_1.onlyWorkspaceProjects(project_graph_1.createProjectGraph());
}
...
function getFullProjectGraphFromHost(host) {
    return project_graph_1.createProjectGraph(undefined, undefined, undefined);
}
...
function updateJsonInTree(path, callback) {
    return (host, context) => {
        if (!host.exists(path)) {
            host.create(path, devkit_1.serializeJson(callback({}, context)));
            return host;
        }
        host.overwrite(path, devkit_1.serializeJson(callback(readJsonInTree(host, path), context)));
        return host;
    };
}
...
function updateWorkspaceInTree(callback) {
    return (host, context = undefined) => {
        ...
        host.overwrite(path, devkit_1.serializeJson(callback(readJsonInTree(host, path), context, host)));
        return host;
    };
}
...
function updateNxJsonInTree(callback) {
    return (host, context) => {
        host.overwrite('nx.json', devkit_1.serializeJson(callback(readJsonInTree(host, 'nx.json'), context)));
        return host;
    };
}
exports.updateNxJsonInTree = updateNxJsonInTree;
function addProjectToNxJsonInTree(projectName, options) {
...
exports.addProjectToNxJsonInTree = addProjectToNxJsonInTree;
...
function insertImport(source, fileToEdit, symbolName, fileName, isDefault = false) {
    ...
    const allImports = find_nodes_1.findNodes(rootNode, ts.SyntaxKind.ImportDeclaration);
    ...
    if (relevantImports.length > 0) {
        ...
        relevantImports.forEach((n) => {
            Array.prototype.push.apply(imports, find_nodes_1.findNodes(n, ts.SyntaxKind.Identifier));
            if (find_nodes_1.findNodes(n, ts.SyntaxKind.AsteriskToken).length > 0) {
        ...
        if (importTextNodes.length === 0) {
            const fallbackPos = find_nodes_1.findNodes(relevantImports[0], ts.SyntaxKind.CloseBraceToken)[0].getStart() ||
                find_nodes_1.findNodes(relevantImports[0], ts.SyntaxKind.FromKeyword)[0].getStart();
            return insertAfterLastOccurrence(imports, `, ${symbolName}`, fileToEdit, fallbackPos);
        }
        return new NoopChange();
    }
    // no such import declaration exists
    const useStrict = find_nodes_1.findNodes(rootNode, ts.SyntaxKind.StringLiteral).filter((n) => n.text === 'use strict');
    let fallbackPos = 0;
