...
function generateGraph(args, affectedProjects) {
    ...
    if (!args.file || args.file.endsWith('html')) {
        html = projectsToHtml(projects, graph, affectedProjects, args.focus || null, args.groupByFolder || false, args.exclude || [], layout, !!args.file && args.file.endsWith('html') ? 'build' : 'serve', args.watch);
    }
    else {
    ...
    if (args.file) {
        ...
        startServer(html, args.host || '127.0.0.1', args.port || 4211, args.watch, affectedProjects, args.focus, args.groupByFolder, args.exclude);
    }
}
...
function startServer(html, host, port = 4211, watchForchanges = false, affected = [], focus = null, groupByFolder = false, exclude = []) {
    ...
    const app = http.createServer((req, res) => {
        ...
        const sanitizePath = path_2.normalize(parsedUrl.pathname).replace(/^(\.\.[\/\\])+/, '');
...
function startWatcher() {
    createFileWatcher(app_root_1.appRootPath, () => {
