...
function validateSchema(schema) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        if (!schema.project && !schema.all) {
            schema.project = (yield enquirer_1.prompt([
                {
...
To upgrade change the version number at the top of ${workspace_1.workspaceConfigName(host.root)} and run 'nx format'.
`);
            throw new Error('v2+ Required');
        }
        yield validateSchema(schema);
        const projects = schema.all
            ? devkit_1.getProjects(host).entries()
            : [[schema.project, devkit_1.readProjectConfiguration(host, schema.project)]];
        for (const [project, configuration] of projects) {
            const configPath = get_project_configuration_path_1.getProjectConfigurationPath(configuration);
            if (host.exists(configPath)) {
                devkit_1.logger.warn(`Skipping ${project} since ${configPath} already exists.`);
                continue;
            }
            devkit_1.writeJson(host, configPath, configuration);
            devkit_1.updateJson(host, devkit_1.getWorkspacePath(host), (value) => {
                value.projects[project] = path_1.dirname(configPath);
                return value;
            });
            devkit_1.updateJson(host, 'nx.json', (value) => {
...
exports.convertToNxProjectSchematic = devkit_1.convertNxGenerator(convertToNxProjectGenerator);
